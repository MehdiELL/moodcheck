...
### AI journal-emotion classifier on GenLayer
...
Submit text; GenLayer validators independently evaluate it with an LLM, agree on a label and score by consensus, and write the result on chain. AI journal-emotion classifier.

**Deployed contract (Testnet Bradbury):** [`0x4b8A0ec099e581B1f1D3a3A80A262f14Ff1B357B`](https://explorer-bradbury.genlayer.com/address/0x4b8A0ec099e581B1f1D3a3A80A262f14Ff1B357B)
...
2. **`evaluate(id)`** - each validator classifies the content
   (one of: positive, mixed, negative) and scores it 0-100, then must **agree** on the label and a similar
   score via the Equivalence Principle.
3. The **label, score, and reasoning** are written on chain.

## The Intelligent Contract

`contracts/moodcheck.py` targets the GenVM Python runner (pinned by hash):
...
genvm-lint check contracts/moodcheck.py
