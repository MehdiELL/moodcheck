...
# MoodCheck - AI journal-emotion classifier on GenLayer.
...
class MoodCheck(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You are an empathetic analyst. Classify the emotional tone of the JOURNAL ENTRY.
...
Classify the CONTENT as exactly one of: positive, mixed, negative.
Also give an integer intensity score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: positive, mixed, negative>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
